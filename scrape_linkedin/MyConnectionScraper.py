...
class MyConnectionScraper(Scraper):
    def scrape(self):
        ...
        self.total_connections = int(
            re.search(r"(\d+)", total_connections_text).group(1)
        )
    ...
    def get_all_connections(self):
        ...
        for el in self.visible_connections:
            ...
            connection["id"] = re.search(r"/in/(.*?)/", connection_link).group(1)
    ...
    def scroll_to_bottom(self):
        ...
        MAX_CONSECUTIVE = 20
        while (
            num_visible_connections < self.total_connections
            and consecutive_same_num < MAX_CONSECUTIVE
        ):
